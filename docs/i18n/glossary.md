...
Source of truth: `docs/en/`. Translate into `docs/es/` and `docs/ar/` only.

## Do not translate

| Item | Notes |
|------|--------|
| `FBCMJ` | Brand abbreviation |
| `Pearson` | Product brand |
| `Pearson Practice English` | App name |
| `Pearson Education, Inc.` | App publisher name on store listings |
| `ESL` | Keep as ESL (may gloss once: “inglés como segundo idioma” / equivalent) |
| `Gmail` | Product name |
| `Google` | Product brand |
| `FBC Guest` | Wi‑Fi SSID |
| `WIFI:…` href values | Exact URI, including password |
| Pearson signup URL | Exact URL |
| Gmail signup URL | Exact URL: `https://accounts.google.com/signup` |
...
|---------|---------|--------|
| Onboarding Guide | Guía de inicio | دليل الإعداد |
| Let's go! | ¡Vamos! | هيا بنا! |
| Change language | Cambiar idioma | تغيير اللغة |
| Join FBC Guest Wi-Fi | Unirse al Wi‑Fi FBC Guest | الانضمام إلى شبكة FBC Guest |
| Create a new Pearson account | Crear una cuenta nueva de Pearson | إنشاء حساب Pearson جديد |
| Create a new Gmail account | Crear una cuenta nueva de Gmail | إنشاء حساب Gmail جديد |
...
Warm, clear, simple sentences for adult ESL learners. Prefer everyday words. Keep numbered steps aligned with English structure.
